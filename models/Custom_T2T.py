...
@register_model
def custom_t2t_vit_14_p(pretrained=False, **kwargs):  # adopt performer for tokens to token
    if pretrained:
        kwargs.setdefault('qk_scale', 384 ** -0.5)
    kwargs = dict(per_layer_do=True, **kwargs)
...
@register_model
def custom_t2t_vit_14_t(pretrained=False, **kwargs):  # adopt transformers for tokens to token
    if pretrained:
        kwargs.setdefault('qk_scale', 384 ** -0.5)
    kwargs = dict(per_layer_do=True, **kwargs)
